...
class VehicleTracker:
    ...
    def __init__(self, track_id, initial_height, current_frame=0):
        ...
        self.last_seen_frame = current_frame  # 마지막으로 감지된 프레임 번호
...
def detect_vehicle_risks(tracked_vehicles: List[Dict[str, Any]], frame_count: int) -> int:
    ...
    for vehicle in tracked_vehicles:
        ...
        # detect_vehicle_risks 함수 내에서 아래 코드를 추가하여 확인
        if track_id not in vehicle_trackers:
            # 새 트래커 생성
            vehicle_trackers[track_id] = VehicleTracker(track_id, height, frame_count)
...
def clean_old_trackers(current_frame):
    ...
    for track_id, tracker in vehicle_trackers.items():
        frame_diff = current_frame - tracker.last_seen_frame
