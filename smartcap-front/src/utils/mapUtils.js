...
// 구글맵용 마커 아이콘 설정 함수
export const getMarkerIcon = (recognizedType, isNew = false) => {
  let color = '#ff9800'; // 기본 주황색
  
  if (recognizedType === 'Falling' || recognizedType === 'FALL') {
    color = '#ff0000'; // 낙상은 빨간색
  } else if (recognizedType === 'Vehicle') {
    color = '#4CAF50'; // 차량은 초록색
  } else if (recognizedType === 'Material') {
    color = '#2196F3'; // 자재는 파란색
  }
...
// 색상을 반환하는 유틸리티 함수
// 색상을 반환하는 유틸리티 함수 수정
export const getAlarmColor = (recognizedType, isNew = false) => {
  if (isNew) return '#ff0000'; // 새 알람은 항상 빨간색
  
  switch(recognizedType) {
    case 'Falling': 
      return '#ff0000'; // 낙상은 빨간색
    case 'Vehicle': 
      return '#4CAF50'; // 차량은 초록색
    case 'Material': 
      return '#2196F3'; // 자재는 파란색
    default: 
