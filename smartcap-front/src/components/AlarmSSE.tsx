...
export default function AlarmSSE() {
  ...
  const setupAlarmSSEConnection = () => {
    try {
      ...
      const es = new EventSource(`${import.meta.env.VITE_API_BASE_URL}/api/sse/alarms/subscribe`, { withCredentials: true });
  ...
  const setupAccidentSSEConnection = () => {
    try {
      ...
      const es = new EventSource(`${import.meta.env.VITE_API_BASE_URL}/api/sse/accident/subscribe`, { withCredentials: true });
